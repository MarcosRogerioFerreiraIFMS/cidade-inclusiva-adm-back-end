 ...
 * - O usuarioId deve ser injetado após a validação, não vir do body da requisição
 * @param {unknown} input - Dados de entrada não tipados (geralmente req.body)
 * @returns {Omit<ComentarioCreateDTO, 'usuarioId'>} DTO validado para criação de comentário (sem usuarioId)
 * @throws {ZodError} Erro de validação se os dados não atenderem ao schema
 */
export function toCreateComentarioDTO(
  input: unknown
): Omit<ComentarioCreateDTO, 'usuarioId'> {
